...
export default function Trabajadores() {
  ...
  const [summary, setSummary] = useState({ workers: 0, movements: 0, total: 0 });
  ...
  const fetchUserDetails = async (user) => {
    try {
      setErr("");
      setLoadingUser(true);
      // Usamos email en userId (el backend acepta uuid o email en el mismo parámetro)
      const data = await listarMovimientosEfectivo({
        start: startYMD,
        end: endYMD,
        estado: "approved",          // que coincida con "Aprobaciones"
        userId: user.email,
      });
      setMovsUser(Array.isArray(data?.rows) ? data.rows : []); // <-- CORREGIDO
    } catch (e) {
      setMovsUser([]);
      setErr(e?.response?.data?.error || e.message || "Error cargando detalle");
    } finally {
  ...
  const fetchAll = async () => {
    try {
      ...
      if (!isYMD(startYMD) || !isYMD(endYMD)) throw new Error("Rango de fechas inválido");

      const data = await listarMovimientosEfectivo({ start: startYMD, end: endYMD });
      ...
      if (selUser?.email) await fetchUserDetails(selUser); // refrescar detalle si estaba abierto
    } catch (e) {
    ...
    <div className="mx-auto w-full max-w-7xl p-3 sm:p-4 md:p-6">
      ...
      <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-semibold sm:text-2xl">Trabajadores</h1>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            onClick={fetchAll}
            disabled={loading}
            className="inline-flex items-center gap-2 rounded-lg bg-emerald-600 px-4 py-2 text-sm font-medium text-white shadow hover:bg-emerald-700 disabled:opacity-60"
          >
      ...
      <div className="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
        <div className="rounded-xl border bg-white p-4 shadow-sm">
          <div className="text-xs text-zinc-500">Trabajadores con movimientos</div>
          <div className="mt-1 text-2xl font-semibold">{summary.workers}</div>
        </div>
        <div className="rounded-xl border bg-white p-4 shadow-sm">
          <div className="text-xs text-zinc-500">Movimientos (aprobaciones)</div>
          <div className="mt-1 text-2xl font-semibold">{summary.movements}</div>
        </div>
        <div className="rounded-xl border bg-white p-4 shadow-sm">
          <div className="text-xs text-zinc-500">Total cobrado</div>
          <div className="mt-1 text-2xl font-semibold">{PEN.format(summary.total)}</div>
        </div>
      </div>
            ...
            <table className="min-w-full text-xs sm:text-sm">
              <thead>
                <tr className="bg-zinc-50 text-zinc-600">
                  ...
                  <th className="px-4 py-2 text-right">Aprobaciones</th>
                  <th className="px-4 py-2 text-right">Total cobrado</th>
              ...
              <tbody>
                {byWorker.length === 0 ? (
                  ...
                  byWorker.map((r, idx) => (
                    <tr key={(r.user_id || r.email || idx).toString()} className="border-t hover:bg-zinc-50">
                      ...
                      <td className="px-4 py-2 text-right">{r.aprobaciones ?? r.count ?? 0}</td>
                      <td className="px-4 py-2 text-right">{PEN.format(Number(r.total || 0))}</td>
                      <td className="px-4 py-2 text-right">{toEsDateTime(r.last)}</td>
        ...
        <div className="rounded-2xl border bg-white shadow-sm">
          <div className="flex flex-wrap items-center gap-2 border-b px-4 py-3">
            ...
            <div className="font-medium">
              {selUser ? `Detalle de: ${selUser.name} (${selUser.email})` : "Selecciona un trabajador"}
            </div>
            <div className="ml-auto max-w-[40%] truncate text-[11px] text-zinc-500">
              {selUser?.userId || ""}
            </div>
          </div>

          {!selUser ? (
            <div className="p-4 text-sm text-zinc-500">Elige un trabajador de la tabla izquierda para ver sus movimientos.</div>
          ) : loadingUser ? (
            <div className="flex items-center gap-2 p-4 text-sm text-zinc-600">
              <Loader2 className="h-4 w-4 animate-spin" /> Cargando detalle…
            </div>
          ) : movsUser.length === 0 ? (
            <div className="p-4 text-sm text-zinc-500">Sin movimientos para el rango seleccionado.</div>
          ) : (
            <div className="overflow-x-auto">
              <table className="min-w-full text-xs sm:text-sm">
                <thead>
                ...
                <tbody>
                  {movsUser.map((m, i) => (
                    <tr key={m.id ?? i} className="border-t">
                      <td className="px-4 py-2">
                        {toEsDateTime(m.approved_at || m.created_at || m.createdAt)}
                      </td>
                      <td className="px-4 py-2 text-right">{PEN.format(Number(m.monto ?? m.amount ?? 0))}</td>
                      <td className="px-4 py-2">{m.pedido_id ?? m.pedidoId ?? "-"}</td>
                      <td className="px-4 py-2">{m.cash_note || m.note || <span className="text-zinc-400">—</span>}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
