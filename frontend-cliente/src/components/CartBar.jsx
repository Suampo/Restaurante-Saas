...
export const CARTBAR_H = 80; // altura referencia
...
export default function CartBar({
  ...
  // 2. cálculo de altura para --cart-bar-h
  useEffect(() => {
    const updateHeight = () => {
      const height =
        itemCount > 0 ? wrapRef.current?.offsetHeight || CARTBAR_H : 0;
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-50 pointer-events-none">
      ...
      <div aria-live="polite" className="sr-only">
        {`Carrito con ${itemCount} ítems, total ${totalStr}`}
      </div>
      ...
      >
        ...
        <div className="block sm:hidden px-4 pb-5 pt-2">
          ...
          >
            ...
            {/* brillo hover */}
            <div className="pointer-events-none absolute inset-0 bg-gradient-to-r from-white/0 via-white/5 to-white/0 opacity-0 transition-opacity duration-500 group-hover:opacity-100" />
          </button>
        </div>

        {/* === DESKTOP/TABLET (>=640px): BARRA FROSTED === */}
