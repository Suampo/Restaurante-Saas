...
const QuantitySelector = ({ quantity, setQuantity, min = 1, max = 99 }) => (
  <div className="inline-flex items-center gap-3 rounded-full bg-gradient-to-r from-neutral-50 to-neutral-100 p-1 shadow-sm ring-1 ring-black/5">
    <button
      ...
      className="grid h-9 w-9 place-items-center rounded-full bg-white text-neutral-700 shadow-sm transition-all hover:scale-105 hover:bg-neutral-50 disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:scale-100"
      aria-label="Disminuir cantidad"
    >
      <Minus className="h-4 w-4" />
    </button>
    <div className="min-w-[2rem] text-center font-bold tabular-nums text-neutral-900" aria-live="polite">
      {quantity}
    </div>
    <button
      type="button"
      ...
      className="grid h-9 w-9 place-items-center rounded-full bg-gradient-to-br from-emerald-600 to-emerald-700 text-white shadow-md shadow-emerald-500/30 transition-all hover:scale-105 hover:shadow-lg disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:scale-100"
...
const OptionItem = ({ option, count, onCountChange, disabled }) => (
  <li className="group relative flex items-center gap-3 p-3 transition-colors hover:bg-gradient-to-r hover:from-emerald-50/50 hover:to-transparent">
    ...
    <div className="flex-1 min-w-0">
      <div className="text-sm font-semibold text-neutral-900 leading-snug break-words">
        {option.name}
      </div>
    ...
    <div className="flex items-center gap-1.5">
      ...
      <button
        ...
        onClick={() => onCountChange(1)}
        disabled={disabled}
        className="grid h-8 w-8 place-items-center rounded-lg bg-gradient-to-br from-emerald-600 to-emerald-700 text-white shadow-md shadow-emerald-500/30 transition-all hover:scale-105 hover:shadow-lg disabled:opacity-30 disabled:cursor-not-allowed disabled:hover:scale-100"
...
export default function ProductSheet({ open, item, onClose }) {
  ...
  const abs = (u) => makeAbs(apiBase, u);

  const overlayRef = useRef(null);
  const closeBtnRef = useRef(null);
  const mainRef = useRef(null);
  const headingId = useMemo(() => `psh-title-${item?.id ?? "x"}`, [item?.id]);

  // 1) Consolidar lista global
  ...
  const catIdsByKey = useMemo(() => {
    ...
    const findIds = (keys) =>
      categories
        .filter((c) => keys.some((k) => norm(c?.nombre || c?.name).includes(k)))
        .map((c) => c.id);
  ...
  const grupos = useMemo(() => {
    const toOpt = (x) => ({
      ...
      image: x.imagen_url ? abs(x.imagen_url) : null,
    });
    ...
    if (extras.length)
      out.push({ key: "extras", title: "Elige tus Toppings Extras", icon: Sparkles, max: 10, items: extras });
    if (acomp.length)
      out.push({ key: "acomps", title: "Acompañamientos", icon: ChefHat, max: 10, items: acomp });
    if (drinks.length)
      out.push({ key: "drinks", title: "Bebidas", icon: ShoppingBag, max: 10, items: drinks });
    return out;
  }, [fullList, catIdsByKey, abs]);
  ...
  const extrasTotal = useMemo(() => {
    const priceMap = new Map(grupos.flatMap((g) => g.items.map((i) => [i.id, i.price])));
    return Object.entries(selected).reduce(
      (sum, [id, count]) => sum + (priceMap.get(Number(id)) || 0) * count,
      0,
    );
  }, [selected, grupos]);

  const total = useMemo(() => basePrice * qty + extrasTotal, [basePrice, qty, extrasTotal]);

  const handleToggleAdd = useCallback(
    (groupId, itemId, delta) => {
  ...
  const addToCart = () => {
    ...
    Object.entries(selected).forEach(([id, count]) => {
      ...
      const extraItem = { id: option.id, nombre: option.name, precio: option.price, imagen_url: null };
      for (let i = 0; i < count; i++) {
        window.dispatchEvent(new CustomEvent("cart:add", { detail: { item: extraItem } }));
      }
    });
  ...
  const imgSrc = item.imagen_url ? abs(item.imagen_url) : FALLBACK_IMG;

  return (
    <div
      ref={overlayRef}
      tabIndex={-1}
      className="fixed inset-0 z-[80] flex items-start justify-center bg-black/70 backdrop-blur-sm"
      role="dialog"
      aria-modal="true"
          ...
          <div className="relative px-4 py-4">
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1 pt-0.5">
                <h1 id={headingId} className="mb-1.5 line-clamp-2 text-base font-bold tracking-tight text-white drop-shadow-md">
                  {item.nombre}
                </h1>
                <div className="inline-flex items-center gap-1.5 rounded-full bg-white/20 px-2.5 py-0.5 backdrop-blur-sm">
                  ...
                  <span className="text-[11px] font-semibold text-white/90">Restaurante</span>
                </div>
              </div>
              ...
              <button
                ref={closeBtnRef}
                onClick={onClose}
                className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-white/20 text-white backdrop-blur-md transition-all hover:bg-white/30 hover:scale-105"
                aria-label="Cerrar"
              >
                <X className="h-4 w-4" />
              </button>
        ...
        >
          ...
          <div className="mb-4 flex items-center gap-3 rounded-xl bg-gradient-to-r from-emerald-50 to-emerald-100/50 p-3 ring-1 ring-emerald-200/50">
            ...
            <div className="flex-1">
              <div className="text-[10px] font-semibold uppercase tracking-wide text-emerald-700">Precio base</div>
              <div className="text-lg font-bold text-emerald-900">{currency(basePrice)}</div>
            </div>
          </div>

          {/* Descripción */}
          {item.descripcion && (
            <p className="mb-5 leading-relaxed text-neutral-600 text-sm">{item.descripcion}</p>
          )}
                ...
                return (
                  <section key={g.key}>
                    <div className="mb-3 flex items-center gap-2.5">
                      ...
                      <div className="flex-1 min-w-0">
                        <h2 className="text-sm font-bold text-neutral-900">{g.title}</h2>
                        <p className="text-[11px] text-neutral-500">
                          Hasta {g.max} · {currentGroupTotal}/{g.max} seleccionadas
                        </p>
                      </div>
                    </div>
                    <ul className="divide-y divide-neutral-100 overflow-hidden rounded-xl border border-neutral-200/70 bg-white shadow-sm">
                      {g.items.map((opt) => (
                        <OptionItem
                          key={`${g.key}-${opt.id}`}
                          option={opt}
                          count={selected[opt.id] || 0}
                          onCountChange={(delta) => handleToggleAdd(g.key, opt.id, delta)}
                          disabled={currentGroupTotal >= g.max}
                        />
                      ))}
                    </ul>
                  </section>
          ...
          <section className="mt-5 mb-4">
            <h2 className="mb-2.5 text-sm font-bold text-neutral-900">Comentarios Adicionales</h2>
