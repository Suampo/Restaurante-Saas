...
router.post('/pedidos/:id/pagado', express.json({ type: '*/*' }), async (req, res) => {
  try {
    ...
    const { amount, pasarela, payment_id, method, status } = req.body || {};
    ...
    if (amount != null) {
      const esperado = Number(pedido.total);
      const recibido = Number(amount);
      if (Math.abs(esperado - recibido) > 0.01) {
        console.warn('[pagado] monto distinto', { pedidoId: id, esperado, recibido });
      }
    }
    ...
    try {
      await supabase.from('pagos').insert([{
        pedido_id   : id,
        monto       : amount != null ? Number(amount) : Number(pedido.total),
      ...
      console.warn('[pagos] no se pudo insertar en public.pagos:', e.message);
    }
