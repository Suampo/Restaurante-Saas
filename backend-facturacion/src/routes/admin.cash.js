...
function toStartISO(d) { return d ? new Date(`${d}T00:00:00Z`).toISOString() : null; }
function toEndISO(d)   { return d ? new Date(`${d}T23:59:59Z`).toISOString() : null; }
...
router.get("/cash-movements", async (req, res) => {
  try {
    ...
    const totalEfectivo = approvedRows.reduce((s, r) => s + Number(r.monto || 0), 0);

    // ---------- 3) Mapear emails -> nombres desde public.usuarios ----------
    const uniqueEmails = Array.from(
      new Set(approvedRows.map(r => String(r.approved_by || "").toLowerCase()).filter(Boolean))
    );
    ...
    if (uniqueEmails.length > 0) {
      ...
      if (eUsers) {
        // No bloquea si falla lectura de nombres
        // console.warn("[admin.cash] usuarios lookup:", eUsers.message);
      } else {
        (urows || []).forEach(u => {
          if (u?.email) nameByEmail[String(u.email).toLowerCase()] = u.nombre || null;
        });
      }
    }
    ...
    const byWorker = Array.from(byMap.values()).map(w => {
      const resolved = w.email ? nameByEmail[String(w.email).toLowerCase()] : null;
      return { ...w, name: resolved || w.name || w.email || "(desconocido)" };
    ...
    if (userId) {
      ...
      details = (rows || [])
        .filter(r => filterIsUuid ? r.approved_by_user_id === userId : r.approved_by === userId)
        .map(r => ({
          id: r.id,
